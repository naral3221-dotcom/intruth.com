        ...
        properties: {
          ...
          args: {
            ...
            properties: {
              ...
              targetType: { type: ['string', 'null'], enum: ['project', 'task', 'meeting', 'routine', 'generic', null] },
              buttonTitle: { type: ['string', 'null'] },
              path: { type: ['string', 'null'], description: 'Relative INTRUTH path for generic sharing, otherwise null' },
...
export class AiAssistantService {
  ...
  private staticToolPlanInstructions() {
    return [
      ...
      'When the user asks to share to KakaoTalk, send to Kakao, copy a briefing, or make a share message, use prepare_kakao_share. Set targetType and the matching ID when sharing an existing project, task, meeting, or routine.',
      'When the user asks for a meeting PDF, PDF file, or PDF share/download, use prepare_meeting_pdf with meetingId. This prepares a client-side PDF share button; do not claim it was sent already.',
      'For prepare_kakao_share, args.content may include a short user-facing message. Keep it concise and Korean. Use args.buttonTitle for a natural CTA such as 회의 보기, 업무 보기, 프로젝트 보기.',
  ...
  private buildToolPlanAssistantResult(input: {
    ...
    return {
      ...
      answer: [
        ...
        '',
        '승인하면 INTRUTH 서버 도구가 권한을 확인한 뒤 실제 데이터 생성/수정 또는 공유 준비를 처리합니다.',
      ].join('\n'),
      highlights,
      suggestedQuestions: [
        '승인 대기 보여줘',
