...
| 6. 승인 기반 쓰기 Agent | ✅ 2차 진행 | ███████░░░ 70% | `AiAgentAction` 모델, 업무 초안 생성, 승인/거절 카드, 승인 후 Task 생성, 실행 로그, 전역 AI 명령 채팅 패널, 대화형 채팅 UX, 후속 실행 버튼 | 회의/프로젝트/루틴 생성 도구, 수정 diff 승인, 서버 tool registry |
