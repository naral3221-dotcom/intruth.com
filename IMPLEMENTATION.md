...
AI 회의 녹음/자료 생성   ████████████████████ 2차 완료 (녹음 업로드, OpenAI 전사, 회의자료 초안 생성/검토/적용)
AI 액션 아이템 업무 전환 ████████████████████ 2차 완료 (업무 후보 편집, Task 생성, 담당자 자동 매칭, 생성 업무 공유)
AI Assistant 읽기 모드  ██████████████████░░ 2차 완료 (범위 선택, 자연어 요약, 사용량 기록, 모바일 기록 재열람)
AI 승인 기반 업무 생성 ████████░░░░░░░░░░░░ 1차 완료 (업무 초안 생성, 승인/거절, 실행 로그, Task 생성)
AI 명령형 에이전트 UI ████████░░░░░░░░░░░░ 1차 완료 (전역 채팅 명령창, 페이지 이동/모달 실행, 승인 대기 처리)
로컬 서버 개발환경       ████████████████████ 1차 완료 (임베디드 PostgreSQL, 서버 모드 env, dotenv)
실서버 API 호환 라우트   ████████████████████ 1차 완료 (teams/routine-tasks 조회 호환)
```
...
| `routes/ai.ts` | 2026-05-01 | Codex | AiTranscriptionService/AiAssistantService 사용, 회의 녹음/자료 초안/업무 전환/AI Assistant |
| `routes/teams.ts` | 2026-04-30 | Codex | 서버 모드 조회 호환 라우트, DB Service 구현 대기 |
| `routes/routineTasks.ts` | 2026-04-30 | Codex | 서버 모드 조회 호환 라우트, DB Service 구현 대기 |

### 미완료
| 라우트 | 상태 | 다음 작업 |
|--------|------|----------|
| `routes/teams.ts` | 조회 호환 라우트만 제공 | Team 모델/Service/Repository 설계 후 쓰기 API 전환 |
| `routes/routineTasks.ts` | 조회 호환 라우트만 제공 | RoutineTask 모델/Service/Repository 설계 후 쓰기 API 전환 |
...
| 승인 기반 쓰기 Agent 확장 | ⬜ 진행 예정 | 회의/프로젝트/루틴 생성 도구와 수정 diff 승인 카드 |
...
- 일반 개발 모드에서 AI API가 잘못된 `localhost:3001`로 향하던 문제를 `127.0.0.1:3002` 기준으로 수정
...
- 로컬 API 포트를 `3002` 기준으로 정리해 기존 3001 포트 충돌 회피
